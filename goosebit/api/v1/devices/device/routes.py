...
from goosebit.device_manager import DeviceManager, get_update_manager
...
async def device_get(_: Request, updater: DeviceManager = Depends(get_update_manager)) -> DeviceResponse:
...
async def device_logs(_: Request, updater: DeviceManager = Depends(get_update_manager)) -> DeviceLogResponse:
    device = await updater.get_device()
    if device is None:
        raise HTTPException(404)
